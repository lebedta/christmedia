...
class PluginCommentCommonForm extends BaseCommentForm
{
  public function setup()
  {
    ...
    $this->widgetSchema['reply_author_id'] = new sfWidgetFormInputHidden();
    ...
    $this->validatorSchema['reply_author_id'] = new sfValidatorPass();
  ...
  {
    $values['body'] = urlencode($values['body']);
    ...
    if(isset($values['reply']) && $values['reply'] != "" && $obj = Doctrine::getTable('Comment')->find($values['reply']))
    {
      ...
      $values['reply_author'] = $values['reply_author'];
      $values['reply_author_id'] = $values['reply_author_id'];
