...
class createScrinshotTask extends sfBaseTask
{
    protected function configure()
    {
        ...
        $this->addOptions(array(
            new sfCommandOption('application', null, sfCommandOption::PARAMETER_REQUIRED, 'FoundersHub', 'frontend'),
            ...
            if($video->getDuration()> 0 )
            {
                ...
                if($count>0)
                {
                    for($i=1; $i<=$count_scrinshot; $i++)
                    {
                        ...
                        $filename= sha1($video->getTitle().rand(11111, 99999));
                        //scrinshot
                        $command="ffmpeg -i ".$video_path.$video->getFile()." -ss ".$duration_L." -vframes 1 ".$path.$filename.$i.".png";
                ...
                {
                    //scrinshot
                    $command="ffmpeg -i ".$video_path.$video->getFile()." -ss 0 -vframes 1 ".$path.$filename.".png";
                    exec($command . ' 2>&1', $output);
                    echo $command;
                    $scrrinshot = new Scrinshot();
